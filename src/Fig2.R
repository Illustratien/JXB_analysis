...
# toolPhD::view_parse('DM["straw,87"]')
pll2 <- purrr::map(si.vec,function(si.nam){
  ...
  pl <- purrr::map(1:3,function(i){
    ...
    if(grepl("\\^",unit0[[i]])){
      ylab1 <- bquote(bolditalic(.(Si.txt))[bold("g, "*.(trnam))]~bold("x"*"("*"10"^"6"*"/"*"ha"*")"))
      xlab1<- bquote(bold(.(trnam)~bold("x"*"("*"10"^"6"*"/"*"ha"*")")))
    }
...
p <- ggcorrplot::ggcorrplot(M, 
  ...
  theme(legend.title     = element_text(size=7),
        ...
        # axis.text.x = element_text(parse=T),
...
lk2 <- data.frame(ori=colnames(M2),
                  # new=c("GY",'DM["straw,87"]',"GCD","HI","S","GL","GN","GpS","GW","post","SN","TGW")
                  new=c("GY",'DM["87,straw"]',"HI","GL","GN","GpS","GW",'DM["61-87,straw"]',
                        # 'DM["straw,61-87"]/DM["spike,87"]',
                        'DM["87-61,all"]',"SN","TGW"
                        )
                  ) %>% 
...
lgd2 <- cowplot::get_plot_component(p2,"guide-box-right",return_all=T)

lgd <- cowplot::get_plot_component(pll2[[1]][["plot_yield"]]+
                             th_lgd+
...
p3 <- cowplot::plot_grid(pll2[[1]][["plot_yield"]]+th_rm,
                         ...
                         label_size = 10)
tiff(filename='result/Fig.2.tiff',
     units="cm",
     width=20,#21.6
     height=14,#16.1
     compression = "lzw",
