...
r.target <- paste(c(".git",".Rproj.user","result",'plot','table','README_files',".quarto","gfm","docs"),
                  collapse='|')
...
x <- lapply(strsplit(dirst, "/"), 
  ...
  group_by(V1,V2) %>%
  mutate(V3 = case_when(n() > 5&(V2=='data')~sprintf("%s files",n()-1),
                        n() > 5&(V2=='src')&(!V3=='fun')~sprintf("%s files",n()-8),
                        T~V3))%>%
  group_by(V1,V2,V3) %>%
  mutate(V2=factor(V2,levels=c("data","src"))) %>% 
